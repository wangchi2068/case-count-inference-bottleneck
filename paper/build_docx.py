...
body('将51个州级单位按周加总构建全美序列：流行初期波段平均周捕获规模约6662例，单步预测相对均方误差降至0.0348（中位数0.0115，N=28），低于单州平台（约010，约降至三分之一）。全国汇总误差较低与空间汇聚并存这一事实，与命题3"扩大基数使各项方差同时衰减"的方向相容；但本对比未单独识别环境抵消、子代波动平均与其他汇总效应的相对贡献。属地化预警所需的州级参数异质性与达峰时间差，则界定了简单汇总的适用边界。全部实证结果见图3。')
